...
$(function () {
    let ua = window.location.href.includes('cncalendar')
    console.log(window.location.href.includes('cncalendar'));
    if (ua) {
        $('.userVip').addClass('hidden');
        $('.downloadBtn').addClass('hidden');
        $('.spliteLine').addClass('hidden');
        $('.aboutLink').addClass('hidden');
        $('.weiboContent').addClass('hidden');
        $('.qqContent').addClass('hidden');
        $('.downloadBtn').addClass('hidden');
        $(".icp").html("@2019-2023 ICP:渝ICP备17001599号-7 ");
        $('.wnlIcon').attr("data-content-after", '');
        $('.wnlIcon').css("background-image", "url(../img_new/logo@3x_hl.png)");
    ...
    var isKLG = window.location.href.includes('appklg.com');
    var linkName = ua ? '' : '/webcalandar'
    var link = isKLG ? location.origin : location.origin + linkName;
    console.log(link);
    $('#quickloginiframe').attr('src', link + "/wnl_login/indexapp.html?parenturl=" + encodeURIComponent(location.href) + "#/phonelogin");
    // $('#quickloginiframe').attr('src', window.location.href + "//www.51wnl.com/webcalandar/wnl_login/indexapp.html?parenturl=" + encodeURIComponent(location.href) + "#/phonelogin");
    // $('#quickloginiframe').attr('src', "//testmobile.51wnl-cq.com/webcalandar/wnl_login/indexapp.html?parenturl=" + encodeURIComponent(location.href) + "#/phonelogin");

    jQuery('input[placeholder]').placeholder();


    //登录
    var subdir = link + '/index.html';
    var returl = `{"retUrl":"${subdir}","plateform":"WechatOpen"}`
    returl = window.btoa(unescape(encodeURIComponent(returl)))
    console.log('subdir', subdir, returl);
    $(".loginIframe").attr("src", `https://open.weixin.qq.com/connect/qrconnect?appid=wx56254671d2e2d8c3&redirect_uri=https%3a%2f%2fservice.51wnl.com%2fauth%2fcallback&response_type=code&scope=snsapi_login&state=${returl}&href=https://www.51wnl.com/webcalandar/css_new/wechatIframe.css`);
    console.log($(".loginIframe").attr("src"));
    ...
    var uploadDataUrl = "//www.51wnl.com/api2/uploaddata.aspx";
    ...
    $(".userVip").click(function () {
        ...
        $(".quickVipContent").removeClass("hidden");
    });
    $(".toBuyVip").click(function () {
        alert("抱歉，支付功能暂未开通");
    });
    ...
    function getVerifyCode(phone, type, imgCode) { //, isAgain
        ...
        //     })
        // };
        var dds = `DataString=${encodeURIComponent(JSON.stringify(tempObj))}`;
        var hsign = window.location.search + dds + 'youloft_2020';
        // CryptoJS.MD5
        $.ajax({
            url: httpProtocol + '//u.51wnl.com/UtilsApi/SendSmsCode' + window.location.search,
            type: 'POST',
            dataType: 'json',
            data: { DataString: JSON.stringify(tempObj) },
            headers: {
                hsign: CryptoJS.MD5(hsign) + ''
            },
            success: function (result) {
                if (result.status !== 200) {
                    setTimeout(function () {
    ...
    function setDayViewInfo(year, month, day) {
        ...
        // console.log(chooseMonthLunarInfo[day - 1].is4jue)
        if (chooseMonthLunarInfo[day - 1].is4jue || chooseMonthLunarInfo[day - 1].is4li) {
            $dayViewYiDescDom.html('-');
            $dayViewJiDescDom.html('大事勿用 结婚 动手术 搬家 开工开业 建房 出行上任 安葬');
        } else {
