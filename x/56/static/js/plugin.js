...
//============================== 阴历属性
function calElement(sYear, sMonth, sDay, week, lYear, lMonth, lDay, isLeap, cYear, cMonth, cDay, term, is4jue, is4li) {
...
function calendar(y, m) {
    ...
    for (var i = 0; i < this.length; i++) {
        ...
        // 4绝日
        var is4jue = false
        var juearr=[2,8,14,20]
        // 4离日 
        var is4li = false
        var liarr=[5,11,17,23]
        if((firstNode-1) === ( i + 1 ) && juearr.indexOf((m + 1) * 2 - 2) != -1) {
            is4jue = true
        }
        if((secondNode-1) === ( i + 1 ) && liarr.indexOf((m + 1) * 2 - 1) != -1) {
            is4li = true
        }

        this[i] = new calElement(y, m + 1, i + 1, nStr1[(i + this.firstWeek) % 7], lY, lM, lD++, lL, cY, cM, cD, term, is4jue, is4li);
...
/*
 CryptoJS v3.1.2
 code.google.com/p/crypto-js_new
 (c) 2009-2013 by Jeff Mott. All rights reserved.
 code.google.com/p/crypto-js_new/wiki/License
 */
...
jQuery.fn.placeholder = function () {
    ...
    if (!placeholdersupport) {
        ...
        inputs.each(function () {
            ...
            placeholder.css({ 'height': height, 'line-height': height + "px", 'position': 'absolute', 'left': '10px', 'top': '0', 'color': "#cecfc9", 'font-size': "12px" });
